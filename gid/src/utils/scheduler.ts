...
export class Scheduler {
  ...
  registerAll(): void {
    ...
    cron.schedule('15 9 * * *', () => this.wrap('x-morning', () => this.agent.postX('morning')), { timezone: TZ });
    cron.schedule('0 10 * * *', () => this.wrap('facebook', () => this.agent.postFacebook()), { timezone: TZ });
    cron.schedule('0 12 * * *', () => this.wrap('x-midday', () => this.agent.postX('midday')), { timezone: TZ });
    cron.schedule('0 15 * * *', () => this.wrap('x-afternoon', () => this.agent.postX('afternoon')), { timezone: TZ });
    cron.schedule('0 17 * * *', () => this.wrap('daily-summary', () => this.agent.sendDailySummary()), { timezone: TZ });
    cron.schedule('0 18 * * *', () => this.wrap('x-evening', () => this.agent.postX('evening')), { timezone: TZ });
